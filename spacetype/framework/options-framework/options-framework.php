...
function optionsframework_add_page() {
	...
	$icon = THEME_WEB_ROOT .'/framework/images/favicon.png';

	$count = '';
	if (sn_is_newer_version()) $count = ' <span class="update-plugins count-1"><span class="update-count">1</span></span>';

	$of_page = add_object_page( $theme_data->Name, $theme_data->Name.$count, $menu['capability'], $menu['menu_slug'], $menu['callback'],  $icon);
	add_submenu_page( 'options-framework', __('Theme Options', 'sn' ), __( 'Theme Options', 'sn' ), 'update_core', 'options-framework', 'optionsframework_page' );

	add_submenu_page( 'options-framework', __( 'Theme Updates', 'sn' ), sprintf( __( 'Theme Updates %1$s', 'sn' ), $count), 'update_core', 'options-framework-update', 'optionsframework_update_page' );


	// Support link/page
	add_submenu_page( 'options-framework', __( 'Support Forums', 'sn'), __('Support Forums', 'sn'), 'update_core', 'options-framework-support', 'optionsframework_support_page');
...
function optionsframework_load_scripts( $hook ) {
	...
	if ( !wp_script_is( 'wp-color-picker', 'registered' ) ) {
		...
		wp_register_script( 'wp-color-picker', OPTIONS_FRAMEWORK_DIRECTORY . 'js/color-picker.min.js', array( 'jquery', 'iris' ), SN_THEME_VERSION );
 ...
 *
 */
function optionsframework_update_page() {
?>

	<div class="wrap">
		<div id="icon-themes" class="icon32"></div>
		<h2><?php _e( 'Theme Updates', 'sn' ); ?></h2>

		<?php
			if ($purchased_theme = sn_is_newer_version()):
				$theme_data = wp_get_theme();
		?>

				<div id="message" class="updated">
					<p><?php printf( __( '<strong>There is a new version of the %s theme available.</strong> You have version %s installed. <a href="%s">Update to version %s</a>.', 'sn' ), $theme_data->Name, $theme_data->Version, admin_url( 'themes.php?theme=spacetype' ), $purchased_theme['new_version']); ?></p>
				</div>

				<p><a class="button button-primary" href="themes.php?theme=spacetype">UPDATE TEMPLATE</a></p>


		<?php else: ?>

		<div id="message" class="updated">
			<p><?php printf( __( '<strong>Your %s theme version is up to date.</strong>', 'sn' ), 'Spacetype'); ?></p>
		</div>
		...
		<?php
			$changelog_url = 'http://theseniores.com/spacetype/assets/changelog.html';
			$trans_key = 'sn_changelog_spacetype';

			$cached_xml = '';

			if ( false === ( $cached_xml = get_transient( $trans_key ) ) ){

				$html = wp_remote_request($changelog_url);
				$html =	wp_remote_retrieve_body($html);

				if ($html){
					set_transient( $trans_key, $html, 3600 ); // Cache for 1 hour
				} else {
					return false;
				}
			} else {
...
function optionsframework_support_page() {
?>
	<script type="text/javascript">
		//<![CDATA[
		window.location.replace("http://theseniores.com/support");
		//]]>
	</script>
<?php
}