...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != "wrap" ) && ( $value['type'] != "wrap_end" ) ) {
		...
		if ( ( $value['type'] != 'heading' ) && ( $value['type'] != 'info') && ( $value['type'] != 'wrap_end' ) ) {
		...
		case "wrap":
			if ( isset( $value['id'] ) )
			{
				$id = 'id="wrap-' . esc_attr( $value['id'] ) . '"';
			}
			$output .= '<div ' . $id . ' class="wrapper">' . "\n";
			break;
